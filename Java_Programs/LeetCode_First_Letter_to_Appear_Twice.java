
// 2351. First Letter to Appear Twice : https://leetcode.com/problems/first-letter-to-appear-twice/
import java.util.HashMap;

public class LeetCode_First_Letter_to_Appear_Twice {
    static char repeatedCharacter(String s) {
        HashMap<Character, Integer> maps = new HashMap<>();
        for (int i = 0; i < s.length(); i++) {
            if (maps.containsKey(s.charAt(i))) {
                if (maps.get(s.charAt(i)) + 1 == 2) {
                    return s.charAt(i);
                } else {
                    maps.put(s.charAt(i), 1 + maps.get(s.charAt(i)));
                }
            } else {
                maps.put(s.charAt(i), 1);
            }
        }
        return Character.valueOf(' ');
    }

    public static void main(String[] args) {
        // String str = "abcdd";
        String str = "abccbaacz";
        String ans = String.valueOf(repeatedCharacter(str));
        System.out.println(ans);
    }
}

// Output
// Input: s = "abccbaacz"
// Output: "c"
// Explanation:
// The letter 'a' appears on the indexes 0, 5 and 6.
// The letter 'b' appears on the indexes 1 and 4.
// The letter 'c' appears on the indexes 2, 3 and 7.
// The letter 'z' appears on the index 8.
// The letter 'c' is the first letter to appear twice, because out of all the
// letters the index of its second occurrence is the smallest.

// Input: s = "abcdd"
// Output: "d"
// Explanation:
// The only letter that appears twice is 'd' so we return 'd'.

// Explanation: Given a string s consisting of lowercase English letters, return
// the first letter to appear twice.

// Note:
// A letter a appears twice before another letter b if the second occurrence of
// a is before the second occurrence of b.
// s will contain at least one letter that appears twice.